                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="excerpt_en" id="excerpt_en" class="form-control" rows="3" 
                                          placeholder="Brief summary for social media and search results (max 160 characters)">{{ old('excerpt_en', $article->excerpt_en) }}</textarea>
                                <div class="d-flex justify-content-between mt-1">
                                    <small class="text-muted">Brief summary (max 160 characters)</small>
                                    <small class="word-count" id="excerpt_en_count">{{ Str::length($article->excerpt_en ?? '') }} / 160</small>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" name="title_km" id="title_km" class="form-control" 
                                       value="{{ old('title_km', $article->title_km) }}" required
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="content_km" id="content_km" class="form-control summernote" rows="15" required
                                          placeholder="សរសេរអត្ថបទរបស់ភាសាខ្មែរ...">{{ old('content_km', $article->content_km) }}</textarea>
                                @error('content_km')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="excerpt_km" id="excerpt_km" class="form-control" rows="3" 
                                          placeholder="សង្ខេីខ្លីសម្រាប់បណ្ដាញសង្សាខ្មែរ និងលទ្ធផលស្វែងរក (អតិបរមា 160 តួអក្សរ)">{{ old('excerpt_km', $article->excerpt_km) }}</textarea>
                                <div class="d-flex justify-content-between mt-1">
                                    <small class="text-muted">សង្ខេីខ្លីសម្រាប់ (អតិបរមា 160 តួអក្សរ)</small>
                                    <small class="word-count" id="excerpt_km_count">{{ Str::length($article->excerpt_km ?? '') }} / 160</small>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @php
                        $rawGallery = $article->getRawOriginal('gallery_images') ?? [];
                        if (is_string($rawGallery)) {
                            $rawGallery = json_decode($rawGallery, true) ?? [];
                        }
                    @endphp
                    
                    @if(count($rawGallery) > 0)
                    <div class="mb-3">
                        <label class="form-label">Current Gallery Images</label>
                        <div id="currentGallery" class="row g-2 mb-3">
                            @foreach($rawGallery as $index => $image)
                            <div class="col-4 col-md-3" id="gallery-item-{{ $index }}">
                                <div class="position-relative" style="border-radius: 8px; overflow: hidden; border: 1px solid #dee2e6;">
                                    <img src="{{ url('/laravel-img/' . $image) }}" class="img-fluid" style="height: 100px; width: 100%; object-fit: cover;">
                                    <div class="position-absolute top-0 end-0 p-1">
                                        <button type="button" class="btn btn-danger btn-sm p-1" onclick="removeGalleryImage('{{ $image }}', {{ $index }})" title="Remove">
                                            <i class="bi bi-x"></i>
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="project_id" id="project_id" class="form-select">
                                    ...
                                    @foreach($projects ?? [] as $project)
                                        <option value="{{ $project->id }}" {{ old('project_id', $article->project_id) == $project->id ? 'selected' : '' }}>
                                            {{ $project->name_en }}
                                        </option>
                                    @endforeach
                                </select>
                                    ...
                                    <div class="card card-body bg-light">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" id="newTagKm-control" placeholder="" class="formបញ្ចូលចំណងជើងឈ្មោះ">
                                            </div>
                                        </div>
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (fileInput.files.length > 0) {
                ...
                if (file.size > maxSize) {
                    e.preventDefault();
                    alert('Image size is too large! Maximum file size is 5MB. Your image is ' + (file.size / 1024 / 1024).toFixed(2) + 'MB.');
                    return false;
                }
            }
        });
...
function removeGalleryImage(imagePath, index) {
    ...
    if (!removedInput) {
        ...
        document.getElementById('articleForm').appendChild(removedInput);
    }
    removedInput.value = JSON.stringify(removedGalleryImages);
}
