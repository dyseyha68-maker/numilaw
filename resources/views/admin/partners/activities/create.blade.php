...
@section('title', __('partner.admin.add_activity'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <a href="{{ route('admin.partners.activities.index', $partnerUniversity->id) }}" class="btn btn-outline-secondary btn-sm mb-2">
            <i class="bi bi-arrow-left me-1"></i> {{ __('partner.form.cancel') }}
        </a>
        <h4 class="fw-bold mb-0">{{ __('partner.admin.add_activity') }}</h4>
        <small class="text-muted">{{ $partnerUniversity->name }}</small>
    </div>
</div>

<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form action="{{ route('admin.partners.activities.store', $partnerUniversity->id) }}" method="POST">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="location" class="form-control" 
                           value="{{ old('location') }}" placeholder="e.g., Phnom Penh, Cambodia">
                    @error('location')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-lg me-1"></i> {{ __('partner.admin.add_activity') }}
                </button>
                <a href="{{ route('admin.partners.activities.index', $partnerUniversity->id) }}" class="btn btn-outline-secondary">
